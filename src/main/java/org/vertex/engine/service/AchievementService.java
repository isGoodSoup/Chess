...
public class AchievementService {
    ...
    private void onMove(TotalMovesEvent event) {
        ...
        if(GameService.getGames() == Games.CHESS && moveCount.get(piece.getID()) < 5 && isQuickWin) {
            unlock(Achievements.QUICK_WIN);
            isQuickWin = false;
        }

        if(GameService.getGames() == Games.CHECKERS && moveCount.get(piece.getID()) < 8 && isQuickWin) {
    ...
    private void onCapture(CaptureEvent event) {
        ...
        if(GameService.getGames() == Games.CHESS && isFirstCapture) {
            unlock(Achievements.FIRST_CAPTURE);
            isFirstCapture = false;
        }

        if(GameService.getGames() == Games.CHECKERS && isFirstCapture) {
    ...
    private void onCastling(CastlingEvent event) {
        if(GameService.getGames() != Games.CHESS) { return; }
        castlingCount++;
        if(castlingCount == 10) {
